...
def get_last_n_checkpoints(args):
    """Return paths to the last n epoch checkpoints in save_dir, sorted by epoch number.
    ...
    for fname in os.listdir(args.save_dir):
        ...
        if not parts:
            continue
        prefix = parts[0]  # e.g. "checkpoint8"
        epoch_str = prefix.replace("checkpoint", "")
        try:
            epoch = int(epoch_str)
    ...
    # Select last n
    last_n = checkpoints[-args.average_n_checkpoints:]

    # Return full paths
    return [os.path.join(args.save_dir, fname) for _, fname in last_n]


def average_checkpoints(args):
    last_checkpoints = get_last_n_checkpoints(args)
    n = len(last_checkpoints)
    assert n == args.average_n_checkpoints, 'checkpoints number unmatched'
    state_dict_full = torch.load(last_checkpoints[0], map_location=lambda s,
                                 l: default_restore_location(s, 'cpu'), weights_only=False)
    state_dict = {k: v.float() / n for k, v in state_dict_full['model'].items()}

    for path in last_checkpoints[1:]:
        state_dict_checkpoint_full = torch.load(path, map_location=lambda s,
                                                l: default_restore_location(s, 'cpu'), weights_only=False)
        state_dict_checkpoint = state_dict_checkpoint_full['model']
        for k in state_dict.keys():
            state_dict[k] += state_dict_checkpoint[k].float() / n
    state_dict_full['model'] = state_dict
    torch.save(state_dict_full, os.path.join(args.save_dir, 'checkpoint_averaged_last{}.pt'.format(n)))
    return state_dict_full


def load_given_checkpoint(args, model, optimizer, checkpoint_file):
    checkpoint_path = os.path.join(args.save_dir, checkpoint_file)
    if os.path.isfile(checkpoint_path):
        state_dict = torch.load(checkpoint_path, map_location=lambda s,
                                l: default_restore_location(s, 'cpu'), weights_only=False)
        model.load_state_dict(state_dict['model'])
