...
@register_model('gqa-transformer')
class TransformerModel(Seq2SeqModel):
    ...
    def add_args(parser):
        ...
        parser.add_argument('--query-groups', type=int, default=8,
                            help='number of query groups, valid with gqa-transformer arch')
    ...
    def build_model(cls, args, src_tokenizer, tgt_tokenizer):
        ...
        encoder = TransformerEncoder(
            ...
            n_query_groups=args.query_groups,
...
class TransformerEncoder(Seq2SeqEncoder):
    ...
    def __init__(self,
        ...
        self.encoder_blocks = nn.ModuleList(
            [EncoderBlock(dim_embed, dropout, n_attention_heads, n_query_groups, dim_ff) for _ in range(n_encoder_layers)])
...
class EncoderBlock(nn.Module):
    ...
    def __init__(self, dim_embed, dropout, n_heads, n_query_groups, dim_ff):
        super(EncoderBlock, self).__init__()
        self.atten = GroupedQueryAttention(n_heads, n_query_groups, dim_embed, dropout)
...
class TransformerDecoder(Seq2SeqDecoder):
    ...
    def __init__(self,
        ...
        self.decoder_blocks = nn.ModuleList(
            [DecoderBlock(dim_embed, n_attention_heads, n_query_groups, dropout, dim_ff) for _ in range(n_decoder_layers)])
        self.norm = nn.RMSNorm(dim_embed)
...
class GroupedQueryAttention(nn.Module):
    def __init__(self, n_heads: int, n_query_groups, dim_embed: int, dropout: float = 0.0):
        super(GroupedQueryAttention, self).__init__()
        # super().__init__()  python 3.x
        assert dim_embed % n_heads == 0  # check the h number
        assert n_heads % n_query_groups == 0, f'{n_heads} heads is not dividable by {n_query_groups} groups'
        ...
        self.group_size = n_heads // n_query_groups

        self.WQ = nn.Linear(dim_embed, dim_embed)
        self.WK = nn.Linear(dim_embed, self.d_k * n_query_groups)
        self.WV = nn.Linear(dim_embed, self.d_k * n_query_groups)
    ...
    def forward(self, x_query, x_key, x_value, mask=None):
        ...
        # query dimension: nbatch * h * seq_len * d_k
        # key, value dimensions: nbatch * g * seq_len * d_k
        query = self.WQ(x_query).view(nbatch, -1, self.h, self.d_k).transpose(1, 2)
        key = self.WK(x_key).view(nbatch, -1, self.n_query_groups, self.d_k).transpose(1, 2)
        value = self.WV(x_value).view(nbatch, -1, self.n_query_groups, self.d_k).transpose(1, 2)
        # 1.5) Expand key and value (g->h) before attention score
        key = key.repeat_interleave(self.group_size, dim=1)  # (B, g, Tk, dk) -> (B, h, Tk, dk)
...
class DecoderBlock(nn.Module):
    ...
    def __init__(self, dim_embed, n_heads, n_query_groups, dropout, dim_ff):
        super().__init__()
        self.atten1 = GroupedQueryAttention(n_heads, n_query_groups, dim_embed)
        self.atten2 = GroupedQueryAttention(n_heads, n_query_groups, dim_embed)
        self.feed_forward = nn.Sequential(
            nn.Linear(dim_embed, dim_ff),
            nn.ReLU(),
            nn.Dropout(dropout),
...
@register_model_architecture('gqa-transformer', 'gqa-transformer')
def base_architecture(args):
    ...
    args.query_groups = getattr(args, 'query_groups', 8)
